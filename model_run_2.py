...
parser.add_argument('--resume', type=bool, help='whether to resume or not')
parser.add_argument('--pos', type=str, help='part of speech to train on')
parser.add_argument('--ignore', type=str, help='features to ignore')
parser.add_argument('--extension', type=str, help='file extension for langfile')
...
if args.extension:
    ...
    language_extension = f'{language}{extension}'
    languagefile_io = os.path.join(filepath, language, language_extension)
else:
...
def subsets_sync(infile, trainfile, testfile, features):
    ...
    with open(infile, 'r') as csvfile2:
        ...
        for row in csvreader2:
            ...
            if ' ' not in row[1]:
                ...
                rowset = set(row[2].split(';'))
                feats = ' '.join(row[2].split(';'))
                if featset.issubset(rowset):
                    testset.write(f'<start> {charword.lower()} <end>\t<start> {lem_string} {feats} <end>\n')
                elif rowset not in cur_forms:
                    trainset.write(f'<start> {charword.lower()} <end>\t<start> {lem_string} {feats} <end>\n')
                """elif ' '.join(features) not in cur_forms:
                    trainset.write(f'<start> {charword.lower()} <end>\t<start> {lem_string} {feats} <end>\n')
                elif cur_forms[(' '.join(features))] != cur_forms[feats]:
                    trainset.write(f'<start> {charword.lower()} <end>\t<start> {lem_string} {feats} <end>\n')"""
            cur_lem = row[0]
            cur_forms.append(row[1])
        trainset.close()
        testset.close()
...
with open(languagefile_io, 'r') as languagefile:
    ...
    for row in langreader:
        if not row: # empty rows
            continue
        if len(row) != 3: # non-triple rows
            continue
        # if the features are not in the set
        cur_pos = row[2].split(';')[0]
        cur_feats = row[2].split(';')
        cur_feats.remove(cur_pos)
        if cur_pos not in pos_feats:
            pos_feats[cur_pos] = set(cur_feats)
        else:
            pos_feats[cur_pos].update(cur_feats)
    if args.pos:
        keep_pos = args.pos
        pos_feats = {key:val for key, val in pos_feats.items() if key == keep_pos}
    for pos in pos_feats:
        combos = itertools.combinations(pos_feats[pos], 2)
        combolist = list(combos)
        comboset = set()
        for i in range(0, len(combolist)):
            comboset.add(frozenset(combolist[i]))
        for feats in list(comboset):
            if '/' in ''.join(feats):
                continue
            if 'LGSPEC' in ''.join(feats):
                continue
            featsfile.write(f"{pos};{';'.join(feats)}\n")
    featsfile.close()
...
if args.resume and resume == True:
    results_str = os.path.join(filepath, language, 'results')
    results_dir = os.fsencode(results_str)
    list_of_files = os.listdir(results_dir)
    for i, file in enumerate(list_of_files):
        list_of_files[i] = file.decode('UTF-8')
    for filename in list_of_files:
        if filename[-11:] == 'results.txt':
            skipfeats = str(filename[:-12]).split('_')
            if skipfeats in featureslist:
                featureslist.remove(skipfeats)
...
for trainfeats in featureslist:
    ...
    subsets_sync(languagefile, trainfile, testfile_io, trainfeats)
    ...
    if total_loss / steps_per_epoch > 0.15:
        print(f'Total loss too high (average {total_loss / steps_per_epoch}), moving to next feature combination')
        continue
    ...
    with tf.device('/GPU:0'):
        with open(testfile, 'r') as testfile:
            ...
            resultsfile.write('Correct Form\tLemma\tSurprisal\n')
            ...
            for row in testreader:
                ...
                for char in features_list:
                    if char.islower() and char != '<start>' and char != '<end>':
                        lemma_list.append(char)
                ...
                try:
                    ...
                    resultsfile.write(f'{correct_form}\t{lemma_string}\t{form_surp}\n')
                    if (i % 1000 == 0):
